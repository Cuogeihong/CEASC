...
class FSAFDYHead(RetinaDYHead):
    ...
    def forward_single(self, x):
        ...
        """
        if self.training:
            cls_score, bbox_pred, active_positions, total_positions, mse_losses = super().forward_single(x)
            return cls_score, self.relu(bbox_pred), active_positions, total_positions, mse_losses
        else:
            if self.return_hards:
                cls_score, bbox_pred, cls_hards, reg_hards = super().forward_single(x)
                return cls_score, self.relu(bbox_pred), cls_hards, reg_hards
            else:
